...
#define GreenColor [MHColorUtils colorWithRGB:0x00a161]

@interface MHGatewayPlugCountdownViewController ()
@property (nonatomic,strong) DragCircularSlider *dragCircular;
//@property (nonatomic,strong) UIButton *startLeftBtn; // 右侧启动
//@property (nonatomic,strong) UIButton *stopBtn; // 停止

@end

@implementation MHGatewayPlugCountdownViewController {
...
- (void)viewDidLoad {
    ...
    _backgroundImageView.backgroundColor = GreenColor;
...
- (void)buildTableSubviews {
    ...
    _countdownTableView.selectedCountdownTable = ^(int index){
        if(!weakself.countdownTimer.isEnabled){
            if (index==3) {
                // 弹出MHTimerPicker
                MHTimerPicker* timerPicker = [[MHTimerPicker alloc] initWithTitle:NSLocalizedStringFromTable(@"mydevice.gateway.sensor.plug.countdown",@"plugin_gateway",@"倒计时") timePicked:^(NSUInteger hour, NSUInteger minute){
