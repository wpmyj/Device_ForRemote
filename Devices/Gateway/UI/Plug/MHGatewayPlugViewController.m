//
//  MHGatewayPlugViewController.m
//  MiHome
//
//  Created by Lynn on 9/18/15.
//  Copyright © 2015 小米移动软件. All rights reserved.
//

#import "MHGatewayPlugViewController.h"
...
@interface MHGatewayPlugViewController () <CountdownDelegate>
...
@implementation MHGatewayPlugViewController
{
    ...
    MHDeviceGatewaySensorLoopData *     _loopData;
}

- (id)initWithDevice:(MHDevice *)device {
    if (self = [super initWithDevice:device]) {
        self.devicePlug = (MHDeviceGatewaySensorPlug* )device;
        [self loadStatus];
        self.isOpen = self.devicePlug.isOpen;
    }
    return self;
}
...
- (void)setIsOpen:(BOOL)isOpen {
    ...
    if(isOpen){
        ...
        if(!self.devicePlug.countDownTimer.isEnabled){
    ...
    if(self.devicePlug.sload_power < 0) self.currentWatNum.text = @"0";
    else if(self.devicePlug.sload_power > 2500) self.currentWatNum.text = @"2500";
...
//    
    ...
    _loopData.device = self.devicePlug;
    [_loopData startWatchingNewData:@"prop_plug" WithParams:@[ @"neutral_0" , @"load_voltage" , @"load_power" ]]; //, @"power"
    _loopData.fetchNewDataCallBack = ^(id respObj){
        if([respObj isKindOfClass:[NSArray class]] &&
           [respObj count] > 2 &&
           [respObj[0] isKindOfClass:[NSString class]]
           ){
            weakself.devicePlug.neutral_0 = respObj[0];
            weakself.devicePlug.load_voltage = [respObj[1] doubleValue];
            weakself.devicePlug.sload_power = [respObj[2] doubleValue];
            
            weakself.devicePlug.isOpen = [weakself.devicePlug.neutral_0 isEqualToString:@"on"] ? 1 : 0;
            if (!weakself.devicePlug.isOpen) weakself.devicePlug.sload_power = 0;
            
            weakself.isOpen = weakself.devicePlug.isOpen;
        }
    };
...
- (void)loadStatus {
    ...
    [self.devicePlug getPropertyWithSuccess:^(id obj){
        weakself.isOpen = weakself.devicePlug.isOpen;
        weakself.currentWatNum.text = [NSString stringWithFormat:@"%0.1f",weakself.devicePlug.sload_power];
        if(weakself.devicePlug.sload_power < 0) weakself.currentWatNum.text = @"0";
        else if(weakself.devicePlug.sload_power > 2500) weakself.currentWatNum.text = @"2500";
        
    } andFailure:nil];

    //电量统计数据
    [self.devicePlug fetchPlugDataWithSuccess:^(id obj){
        weakself.todayCountNum.text = [NSString stringWithFormat:@"%0.1f",weakself.devicePlug.pw_day];
        weakself.monthCountNum.text = [NSString stringWithFormat:@"%0.1f",weakself.devicePlug.pw_month];

    } failure:nil];

    //插座定时数据
    [self.devicePlug getTimerListWithID:TimerIdentify Success:nil failure:nil];
    [self.devicePlug getTimerListWithID:CountDownIdentify Success:^(id v){
...
- (void)setPwHour:(NSInteger)pwHour {
    _pwHour = pwHour;
    if(pwHour) [self setCountDownLabel];
}

- (void)setPwMinute:(NSInteger)pwMinute {
    _pwMinute = pwMinute;
    if(_pwHour || pwMinute) [self setCountDownLabel];
}
...
- (void)buildSubviews {
    ...
    if(self.isOpen) _bgPlug.image = [UIImage imageNamed:@"gateway_plug_bgon"];
    ...
    if(_isOpen) _imgTitle.text = NSLocalizedStringFromTable(@"mydevice.gateway.sensor.plug.ison", @"plugin_gateway", nil);
    else _imgTitle.text = NSLocalizedStringFromTable(@"mydevice.gateway.sensor.plug.isoff", @"plugin_gateway", nil);
...
- (void)updatePowerTimer {
    ...
    [self.devicePlug getTimerListWithID:CountDownIdentify Success:^(id obj){
        [weakself.devicePlug fetchCountDownTime:^(NSInteger hour, NSInteger minute) {
            weakself.pwHour = hour;
            weakself.pwMinute = minute;
        }];
    ...
    [self.devicePlug getPropertyWithSuccess:^(id objs){
        weakself.isOpen = weakself.devicePlug.isOpen;
        weakself.currentWatNum.text = [NSString stringWithFormat:@"%0.1f",weakself.devicePlug.sload_power];
        if(weakself.devicePlug.sload_power < 0) weakself.currentWatNum.text = @"0";
        else if(weakself.devicePlug.sload_power > 2500) weakself.currentWatNum.text = @"2500";
...
- (void)setCountDownLabel {
    if(self.devicePlug.countDownTimer.isEnabled){
        BOOL isOn = YES;
        if(self.devicePlug.countDownTimer.isOnOpen) isOn = NO;
        else if(self.devicePlug.countDownTimer.isOffOpen) isOn = YES;
        
        if (self.pwHour<1) {
            _imgTitle.text = [NSString stringWithFormat:@"%ld%@%@%@",(long)self.pwMinute, kMINUTE, kREAR, isOn ? kOFF : kON];
        }
        else {
            if (self.pwMinute==0) { // 0分钟，只显示小时
                _imgTitle.text = [NSString stringWithFormat:@"%ld%@%@%@",(long)self.pwHour, kHOUR, kREAR, isOn ? kOFF : kON];
            }
            else {
                _imgTitle.text = [NSString stringWithFormat:@"%ld%@%ld%@%@%@",(long)self.pwHour, kHOUR, (long)self.pwMinute, kMINUTE, kREAR, isOn ? kOFF : kON];
            }
        }
    }
}
...
- (void)defaultIcon {
    if(self.isOpen) [self.imgPlugLogo setImage:[UIImage imageNamed:@"gateway_plug_on"] forState:UIControlStateNormal];
...
- (void)openCountDown {
    ...
    countdownVC.countdownTimer = self.devicePlug.countDownTimer;
    countdownVC.hour = self.devicePlug.countDownTimer.isEnabled ? self.pwHour : 0;
    countdownVC.minute = self.devicePlug.countDownTimer.isEnabled ? self.pwMinute : 0;
...
- (void)countdownDidStart:(MHDataDeviceTimer*)countdownTimer {
    ...
    countdownTimer.identify = CountDownIdentify;
    ...
    [self.devicePlug editTimer:countdownTimer success:^(id obj) {
        weakself.devicePlug.countDownTimer = countdownTimer;
...
- (void)countdownDidDelete:(MHDataDeviceTimer *)countdownTimer {
    ...
    [self.devicePlug deleteTimerId:countdownTimer.timerId success:^(id obj) {
        ...
        weakself.devicePlug.countDownTimer = nil;
        [weakself.devicePlug saveTimerList];
...
- (void)openTimerView {
    ...
    XM_WS(weakself);
    self.tVC = [[MHGatewayTimerSettingNewViewController alloc] initWithDevice:self.devicePlug andIdentifier:TimerIdentify];
...
- (void)onMore:(id)sender {
    ...
    NSArray *titlesArray =  @[ ifttt, logoChange, deviceProtect, strChangeTitle, strShowMode, strFAQ, strFeedback ];
    NSLog(@"%@", NSStringFromClass([self.devicePlug class]));
    
    if ([NSStringFromClass([self.devicePlug class]) isEqualToString:@"MHDeviceGatewaySensorCassette"]) {
            titlesArray = @[ ifttt, deviceProtect, strChangeTitle, strShowMode, strFAQ, strFeedback ];
        ...
        [[MHPromptKit shareInstance] showPromptInView:self.view withHandler:^(NSInteger buttonIndex) {
            switch (buttonIndex) {
                ...
                case 5: {
                    ...
                    [weakself gw_clickMethodCountWithStatType:[NSString stringWithFormat:@"actionSheet设置列表显示:%ld",(long)buttonIndex]];
