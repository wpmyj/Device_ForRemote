...
#define TimerCellId             @"MHACPartnerTimerCell"
#define TableViewCellHeight     60

@interface MHACPartnerTimerView () <UIActionSheetDelegate>

@end

@implementation MHACPartnerTimerView
{
    MHDeviceAcpartner*          _acpartner;
    NSArray*                    _timerList;
...
- (void)buildSubviews {
    ...
    [_tableview registerClass:[MHACPartnerTimerCell class] forCellReuseIdentifier:TimerCellId];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_acpartner.shareFlag == MHDeviceUnShared) {
...
//    NSLog(@"定时列表%@", _timerList);
//    NSLog(@"行数%ld", indexPath.row);

    MHDataDeviceTimer* timer = _timerList[indexPath.row];
    MHACPartnerTimerCell* cell = [tableView dequeueReusableCellWithIdentifier:TimerCellId];
    [cell configureWithDataObject:timer acpartner:_acpartner];
    XM_WS(weakself);
    cell.onSwitch = ^(MHDataDeviceTimer *editTimer) {
        [weakself updateTimerMonthAndDayForRepeatOnceType:editTimer];
        if (weakself.onModifyTimer) {
            weakself.onModifyTimer(editTimer, NO);
        }
    };
    return cell;
}
