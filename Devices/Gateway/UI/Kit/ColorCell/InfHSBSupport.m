...
CGImageRef createSaturationBrightnessSquareContentImageWithHue( float hue )
{
	...
	for( int s = 0 ; s < 256 ; ++s ) {
		...
		unsigned int r_hs = 255 - blend( s, r_s );
		unsigned int g_hs = 255 - blend( s, g_s );
		unsigned int b_hs = 255 - blend( s, b_s );
...
CGImageRef createHSVBarContentImage( InfComponentIndex barComponentIndex, float hsv[ 3 ] )
{
	...
	UInt8* ptr = (UInt8* )CGBitmapContextGetData( context );
	if( ptr == nil ) {
		CGContextRelease( context );
		return nil;
	}
	...
	for( int x = 0 ; x < 256 ; ++x ) {
        ...
        //中间加一段，渐变到白色的；而选择的加入的点的位置大概在黄绿色间
        if (x == 44){
            
            //x从44断开
            //hsv[0] 从 44 加到 54 变为白色
            for (int i = 44 ; i < 54 ; i++){
                hsv[ barComponentIndex ] = (float) i / 255.0f;
                
                //sat 从 hsv[1] ~  0
                //i   从   44   ~  54
                CGFloat sat = (fabs(i - 54.0)) / 10.0  * hsv[1];
                
                HSVtoRGB( hsv[ barComponentIndex ] * 360.0f, sat, 1, &r, &g, &b );
            ...
            //中间在多加一些白色带
            for (int i = 0 ; i<4 ; i++){
                ptr[ 0 ] = (UInt8) ( 1 * 255.0f );
                ptr[ 1 ] = (UInt8) ( 1 * 255.0f );
                ptr[ 2 ] = (UInt8) ( 1 * 255.0f );
                ptr += 4;
            }
            
            //hsv[0] 从 54 减到 44 变为断开处的颜色
            for (int i = 54 ; i> 44 ; i-- ){
                hsv[ barComponentIndex ] = (float) i / 255.0f;
