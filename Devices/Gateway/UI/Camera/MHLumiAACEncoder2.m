...
@implementation MHLumiAACEncoder2
...
-(instancetype)init{
    if ( self = [super init]) {
        ...
        self.path = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"test.aac"];
...
- (void)setUpConverter:(CMSampleBufferRef)sampleBuffer{
    ...
    outAudioStreamBasicDescription.mSampleRate = inAudioStreamBasicDescription.mSampleRate/2;
    // 填充输出的音频格式
