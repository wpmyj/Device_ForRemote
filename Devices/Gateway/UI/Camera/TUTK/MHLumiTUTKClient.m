...
- (MHLumiTUTKClient *)initWithConfiguration:(MHLumiTUTKConfiguration *)configurarion{
    ...
    if (self){
        ...
        _clientQueue = dispatch_get_global_queue(0, 0);
...
- (void)initConnectionWithCompletedHandler:(void (^)(MHLumiTUTKClient *, int))completedHandler{
    ...
    bool (^logMessageAndReturn)(NSString *message, int redCode) = ^(NSString *message, int retCode){
        if (retCode < 0) {
...
- (void)startAndfetchTalkBackServIdWithJsonString:(NSString *)jsonString
    ...
    dispatch_async(self.clientQueue, ^{
        ...
        if (weakself.talkbackAVChannelId < 0){
            weakself.talkbackAVChannelId = IOTC_Session_Get_Free_Channel(weakself.sessionId);
    ...
    while (1) {
        ...
        if (todoDatas.count>=4) {
            while (todoDatas.count >= 4) {
                ...
                NSLog(@"发送后，当前数：%lu",(unsigned long)todoDatas.count);
...
//            if (_aacDataSources.count >= 4){
//                while (_aacDataSources.count >= 4) {
//                    NSMutableData *fullData = [NSMutableData data];
//                    for (int index = 0; index < 4; index ++) {
//                        NSData *todoData = [_aacDataSources objectAtIndex:0];
//                        [_aacDataSources removeObject:todoData];
//                        [fullData appendBytes:todoData.bytes length:todoData.length];
//                    }
//                    __weak typeof(self) weakself = self;
//                    int infoLength = 4;
//                    char *info = malloc(sizeof(char) * infoLength);
//                    NSLog(@"发送后，当前数：%lu",(unsigned long)_aacDataSources.count);
//                    int retcode = avSendAudioData(weakself.talkbackServiceId, fullData.bytes, (int)fullData.length, info, infoLength);
//                    if (retcode < 0){
//                        NSLog(@"发送失败");
//                    }else{
...
- (BOOL)proactiveFetchVideoDataWithABAVFrame:(AVFrame *)frame frameInfo:(LumiTUTKFrameInfo *)frameInfo gotPicturePtr:(int *)pictureCout{
    ...
    retcode = avRecvFrameData2(self.avChannelId, videoBuffer, kVideoBufSize, &pnActualFrameSize, &pnExpectedFrameSize, (char *)&todoFrameInfo, sizeof(LumiTUTKFrameInfo), &pnActualFrameInfoSize, &_prevFrameIndex);
...
//        CFTimeInterval diff = CFAbsoluteTimeGetCurrent() - _audioTimeStamp;
//        if (!(diff >= 0.020)){
//            NSLog(@"usleep %f",diff);
