...
@interface MHGatewayInfoView () <UITableViewDelegate,UITableViewDataSource,UIActionSheetDelegate>
...
@implementation MHGatewayInfoView
{
...
- (id)initWithFrame:(CGRect)frame
    ...
    if (self = [super initWithFrame:frame]) {
        sensorTypeName = @{
                           ...
                           @"MHDeviceGatewaySensorNatgas" : NSLocalizedStringFromTable(@"mydevice.gateway.defaultname.natgas", @"plugin_gateway", nil),
                           @"MHDeviceGatewaySensorSmoke" : NSLocalizedStringFromTable(@"mydevice.gateway.defaultname.smoke", @"plugin_gateway", nil),
                           };
        _gateway = gateway;
...
- (void)buildSensors {
    ...
    for (MHDeviceGatewayBase *sensor in _infoSensors){
        ...
        if([sensor isKindOfClass:[MHDeviceGatewayBase class]]) [sensor buildServices];
...
- (void)fetchLatestLog {
    ...
    for (NSInteger i = 0; i < propIndex; i++) {
        ...
        [_gateway gePropDevices:devices success:^(id obj) {
            [weakself.tableView reloadData];
