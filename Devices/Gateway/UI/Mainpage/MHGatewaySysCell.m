...
@implementation MHGatewaySysCell
...
- (void)configureWithDataObject:(id)object {
    if(![object isKindOfClass:NSClassFromString(@"MHDataIFTTTRecord")] && ![object isKindOfClass:NSClassFromString(@"MHDataIFTTTRecomRecord")]){
        _dataObject = object;
        [self buildSubviews];
    }
...
- (void)buildSubviews {
    ...
    _icon.image = [UIImage imageNamed:[_dataObject valueForKey:@"icon"]];
    
    CGRect nameFrame = CGRectMake(90, 16, WIN_WIDTH - 100, 20);
    if(!_nameLabel) {
    ...
    _nameLabel.text = [_dataObject valueForKey:@"name"];
    
    CGRect detailFrame = CGRectMake(90, 35, WIN_WIDTH - 150, 20);
    if(!_detailLabel){
    ...
    _detailLabel.text = [_dataObject valueForKey:@"detail"];
...
//    _icon.image = [UIImage imageNamed:@"ift_record_icon"];
    _icon.image = nil;
    ...
    CGRect nameFrame = CGRectMake(20, 26, WIN_WIDTH - 140, 20);
    if(!_sceneNameLabel) {
    ...
    CGRect detailFrame = CGRectMake(WIN_WIDTH - 100, 26, 60, 20);
