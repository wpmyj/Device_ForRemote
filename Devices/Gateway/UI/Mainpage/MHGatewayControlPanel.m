...
@interface MHGatewayControlPanel () <UIActionSheetDelegate,UIAlertViewDelegate>
...
@implementation MHGatewayControlPanel
{
}
...
- (void)fetchServices {
    ...
    for (MHDeviceGatewayBase *sensor in _controlDevices){
        if([sensor isKindOfClass:[MHDeviceGatewayBase class]]){
...
- (void)longPressAciton:(NSInteger)index {
    ...
    UIActionSheet *action = [[UIActionSheet alloc] initWithTitle:service.serviceName
                                                        delegate:self
    ...
    [action addButtonWithTitle:NSLocalizedStringFromTable(@"mydevice.actionsheet.more", @"plugin_gateway", @"更多")];
